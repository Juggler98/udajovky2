...
//    }
    ...
    public void setInterval(TTTreeNode<K, T> node, K start, K end) {
        ...
        if (node.isThreeNode()) {
            if ((start.compareTo(node.getDataL().getKey()) > 0 && start.compareTo(node.getDataR().getKey()) < 0) || (end.compareTo(node.getDataL().getKey()) > 0 && end.compareTo(node.getDataR().getKey()) < 0)) {
